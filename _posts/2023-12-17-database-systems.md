...
因为 SQL 是声明式的，查询只告诉数据库管理系统（DBMS）要计算什么，而不是如何计算。因此，DBMS 需要将 SQL 语句翻译成一个可执行的查询计划。
但是，执行查询计划中每个操作符的方法不同（例如，连接算法），这些计划在性能上会有所不同。DBMS 优化器的任务是为给定的查询选择一个最优的计划。

从高层次说，查询优化有两个策略：
- 使用静态规则，或启发式方法。启发式方法将查询的部分与已知模式匹配，以组装一个计划。通过转换查询以消除低效。这些规则无需检查数据本身，但可能需要查阅catalog以理解数据的结构。
- 使用基于成本的搜索来读取数据并估计等效计划的成本。成本模型选择成本最低的计划。

查询优化是构建 DBMS 最困难的部分。一些系统尝试应用机器学习来提高优化器的准确性和效率，但目前还没有DBMS实际使用基于这种技术的优化器。

#### 逻辑计划与物理计划

优化器生成一个逻辑代数表达式到最优等效物理代数表达式的映射。逻辑计划大致相当于查询中的关系代数表达式。

物理操作符定义了一个特定的执行策略，使用查询计划中不同操作符的访问路径。物理计划可能取决于处理的数据的物理格式（即排序、压缩）。

逻辑计划与物理计划之间并不总是存在一对一的映射。
...
图 1：架构概述 - 应用程序连接到数据库系统并发送 SQL 查询，该查询可能会被重写为不同的格式。SQL 字符串被解析成tokens来构建语法树。binder通过查询system catalog，将语法树中的命名对象转换为内部标识符。binder输出一个逻辑计划，该计划可以提供给树重写器以获取额外的schema信息。逻辑计划被交给优化器，优化器选择最高效过程来行计划。

图 2：谓词下推 - 与其在连接（join）之后执行过滤（filter），可以更早地应用过滤，以便传递更少的元素进入过滤过程。

投影操作的优化包括：
- 尽可能早地执行投影以创建更小的元组并减少中间结果（投影下推）。
- 只投影出请求或需要的属性，排除其他所有属性。

图 3：投影下推 - 由于查询只请求学生姓名和 ID，数据库管理系统（DBMS）可以在应用连接（join）之前移除除了这两个字段之外的所有列。

查询重写优化包括：
- 移除不可能或不必要的谓词。在这种优化中，数据库管理系统（DBMS）省略了对那些结果在表中每个元组上都不变的谓词的评估。绕过这些谓词可以减少计算成本。
- 合并谓词
- 通过解关联和/或展平嵌套子查询来重写查询
- 分解嵌套查询并将结果存储到临时表中

图 4：合并谓词 - 查询 1 中的 WHERE 谓词存在冗余，因为它搜索的是 1 到 150 之间的任何值。查询 2 展示了一种更简洁的方式来表达查询 1 中的请求。
图 5：子查询优化 - 重写 通过将子查询重写为 JOIN，前者查询可以被重写为后者查询。以这种方式移除嵌套层级实际上使查询变平。
图 6：子查询优化 - 分解 - 对于包含子查询的复杂查询，数据库管理系统（DBMS）的优化器可能会将原始查询分解成多个块，并逐个优化每个块。在这个例子中，优化器通过将嵌套查询提取出来形成自己的查询，然后使用这个结果来实现原始查询的逻辑。

连接消除：连接（JOIN）操作的顺序是查询性能的关键决定因素。枚举所有可能的连接顺序是低效的，因此连接顺序优化需要一个成本模型。然而，我们仍然可以通过启发式优化方法消除不必要的连接。

图 7：连接消除 - 查询 1 中的连接是浪费的，因为在 A 中的每个元组都必须存在于 A 中。查询 1 可以改写为查询 2。

### 成本估计

DBMS 使用成本模型来估计执行计划的成本。这些模型评估查询的等效计划，以帮助 DBMS 选择最优化的一个。

查询的成本取决于几个基础指标，分为物理成本和逻辑成本，包括：
- CPU：成本小，但难以估计
- 磁盘I/O：块传输的数量
- 内存：使用的 DRAM 量
- 网络：发送的消息数量

对于查询的所有有效计划进行枚举对于优化器来说太慢了。对于连接操作，它们是可交换和可结合的，每个 n 路连接有 4^n 种不同的排序方式。优化器必须限制他们的搜索空间以便高效工作。

为了近似查询的成本，DBMS 在他们的内部catalog中维护有关表、属性和索引的内部统计信息。不同系统以不同方式维护这些统计信息。大多数系统尝试避免运行时计算，通过维护一个内部统计信息表。这些内部表可以在后台更新。
...
有了上述信息，优化器可以推导出选择基数(selection cardinality): SC(A, R) 统计信息。
选择基数: 给定 $\frac{N_R}{V(A,R)}$ 的情况下，属性 A 的值的平均记录数。请注意，这假设数据均匀性。这个假设通常是错误的，但它简化了优化过程。

#### 选择统计

选择基数可以用来确定给定输入将选择的元组数量。

对于唯一键的等值谓词，估计很简单（见图 8）。一个更复杂的谓词示例见图 9。

图 8：简单谓词示例 - 在这个例子中，确定使用哪个索引很容易，因为查询包含一个唯一键的等值谓词。

图 9：复杂谓词示例 - 更复杂的谓词，如范围或连接（合取），更难以估计，因为必须以非常规的方式结合这些谓词的选择基数。

谓词 P 的选择性（sel）指符合条件元组的比例。用于计算选择性的公式取决于谓词的类型。复杂谓词的选择性很难准确估计，这可能会给某些系统带来问题。

一个选择性计算的示例见图 10。

图 10：否定查询的选择性示例 - 否定查询的选择性是通过从 1 中减去正面查询的选择性来计算的。在这个例子中，计算出的答案是 4/5，这是准确的。

注意到谓词的选择性等同于该谓词的概率。这允许在许多选择性计算中应用概率规则。这在处理复杂谓词时特别有用。例如，如果我们假设参与合取的多个谓词是独立的，我们可以将合取的总选择性计算为各个谓词选择性的乘积。


#### 选择性计算假设

在计算谓词的选择基数时，使用以下三个假设:
- 均匀数据：值的分布（除了高频项）是相同的。
- 独立谓词：属性上的谓词是独立的。
- 包含原理：连接键的域重叠，使得内表中的每个键也将存在于外表中。

这些假设通常不被真实数据满足。例如，相关属性打破了谓词独立的假设。

### 直方图

真实数据通常是偏斜的，很难做出假设。然而，存储数据集中的每一个值是昂贵的。一种减少存储数据所用内存的方法是通过直方图将值分组。

图 11：等宽直方图：第一个图表显示了整个数据集的原始频率计数。第二个图表是一个等宽直方图，它将相邻键的计数合并在一起，以减少存储开销。

另一种方法是使用等深度直方图，它改变桶的宽度，使得每个桶的总出现次数大致相同。

图 12：等深度直方图 - 为了确保每个桶中大约有相同数量的计数，直方图改变了每个桶的范围。

一些系统可能使用草图(sketches)来生成数据集的近似统计信息。

### 抽样

DBMS 可以使用抽样技术，将谓词应用于具有类似分布的表的较小副本（见图 13）。当底层表的变更量超过某个阈值（例如，元组的 10%）时，DBMS 会更新样本。

图 13：抽样 - DBMS 可以使用表的一个子集来推导谓词的选择性，而不是使用表中的十亿个值。
...
对于单关系查询计划，最大的障碍是选择最佳访问方法（即顺序扫描、二分搜索、索引扫描等）。大多数新的数据库系统只使用启发式方法，而不是复杂的成本模型，来选择访问方法。

对于 OLTP 查询，这尤其简单，因为它们是可搜索的（sargable: Search Argument Able），这意味着存在一个最佳索引可以为查询选择。这也可以通过简单的启发式方法实现。

### 多关系查询计划

对于多关系查询计划，随着连接数量的增加，可选计划的数量迅速增长。因此，限制搜索空间以便在合理的时间内找到最优计划非常重要。有两种方法来处理这个搜索问题：

- 自底向上：从零开始，然后构建计划以获得你想要的结果。例子：IBM System R, DB2, MySQL, Postgres, 大多数开源 DBMS。
- 自顶向下：从你想要的结果开始，然后向下工作以找到最优计划。例子：MSSQL, Greenplum, CockroachDB, Volcano

#### 自底向上优化示例 - System R

使用静态规则执行初始优化。然后使用动态规划来确定表的最佳连接顺序，使用分而治之的搜索方法。

- 将查询分解成块，并为每个块生成逻辑操作符。
- 对于每个逻辑操作符，生成一组实现它的物理操作符。
- 然后，迭代地构建一个“左深”树，以最小化执行计划的估计工作量。

#### 自顶向下优化示例 - Volcano

从我们希望查询成为的逻辑计划开始。执行分支限界搜索，通过将逻辑操作符转换为物理操作符来遍历计划树。
